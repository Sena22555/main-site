...
import { ArrowUpRight, ChevronLeft, ChevronRight, Menu, MoveUpRight, Quote, Star, X, Sparkles, Utensils, Activity, ShieldAlert, Bot, Wrench, Send, Plus, MessageSquare, Circle } from 'lucide-react';
import { createRoot } from 'react-dom/client';
import { useInViewAnimation } from './hooks/useInViewAnimation';
import './styles.css';
import './cinematic-mobile.css';

// Curated gallery: balanced plates, everyday food choices and fresh produce only.
// Technical screenshots, duplicates and visually mismatched dishes stay out of the UI.
const gallery = [5, 6, 7, 9, 10, 23, 26, 27, 29, 34, 35]
  .map((index) => `/gallery/inbound/${String(index).padStart(2, '0')}.jpg`);
const testimonials = [
  ['Участница программы', 'Результат требует подтверждения', 'За время программы минус 6 кг — и это без голода! Наталья всегда на связи, поддерживает и мотивирует в самые сложные моменты.', 'М'],
  ['Участница программы', 'Результат требует подтверждения', 'Я уже пробовала кучу диет, но здесь совсем другой подход. Наставник реально помогает, и ты не бросаешь на полпути.', 'А'],
  ['Участница программы', 'Результат требует подтверждения', 'Это не просто диета — это образ жизни. После программы стало намного понятнее, как собирать рацион и не возвращаться к хаосу.', 'Е'],
  ['Участница программы', 'Результат требует подтверждения', 'Программа прошла на одном дыхании! Наталья каждый день на связи, всегда ответит и поддержит.', 'О'],
  ['Участница программы', 'Результат требует подтверждения', 'Никогда не думала, что смогу похудеть без жёстких ограничений. Питание стало понятнее, а поддержка помогает держать темп.', 'И'],
];
const stages = [
...
function ApplicationForm() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (hasConditions !== 'no') {
      setStatus(hasConditions === 'yes' ? 'При хронических заболеваниях мы не можем проводить программу. Пожалуйста, обсуди питание с лечащим врачом.' : 'Ответь, пожалуйста, на вопрос о хронических заболеваниях.');
      return;
    }
    setSending(true);
...
type TeamAgent = { id: string; name: string; role: string; initials: string; color: string; status: string; description: string; tools: string[]; model: string; lastMessage: string };
const teamAgents: TeamAgent[] = [
  { id: 'kent', name: 'Кентик', role: 'Главный координатор', initials: 'К', color: '#b8d9c9', status: 'Онлайн', description: 'Помогаю разложить задачу на шаги, выбрать нужного агента и довести результат до готового вида.', tools: ['Файлы и код', 'Веб-поиск', 'Терминал', 'GitHub'], model: 'GPT-5.6 Luna', lastMessage: 'Готов подключить нужного специалиста' },
  { id: 'researcher', name: 'Разведчик', role: 'Исследование и поиск', initials: 'Р', color: '#f1d1a8', status: 'Готов', description: 'Ищу актуальную информацию, сравниваю источники и собираю короткие выводы без информационного шума.', tools: ['Веб-поиск', 'Чтение страниц', 'PDF-анализ'], model: 'Claude Sonnet 4.6', lastMessage: 'Могу проверить факты и собрать источники' },
  { id: 'developer', name: 'Разработчик', role: 'Код и интерфейсы', initials: 'Р', color: '#c8c4e9', status: 'Готов', description: 'Пишу и проверяю код, собираю интерфейсы, нахожу ошибки и объясняю технические решения человеческим языком.', tools: ['Файлы и код', 'Терминал', 'Скриншоты', 'Git'], model: 'GPT-5.4 Mini', lastMessage: 'Могу открыть проект и начать работу' },
  { id: 'editor', name: 'Редактор', role: 'Тексты и структура', initials: 'Е', color: '#e7b9b0', status: 'Готов', description: 'Превращаю черновики в ясные тексты: лендинги, посты, инструкции, сценарии и структуру продукта.', tools: ['Файлы', 'Анализ текста', 'Переписывание'], model: 'Claude Haiku 4.5', lastMessage: 'Готов отредактировать любой текст' },
];
function TeamsPage() {
  const [selectedId, setSelectedId] = useState(teamAgents[0].id);
  const [draft, setDraft] = useState('');
  const [messages, setMessages] = useState<Record<string, { from: 'agent' | 'you'; text: string }[]>>(() => Object.fromEntries(teamAgents.map((agent) => [agent.id, [{ from: 'agent', text: agent.lastMessage }]])));
  const selected = teamAgents.find((agent) => agent.id === selectedId) || teamAgents[0];
  const sendMessage = () => { const text = draft.trim(); if (!text) return; setMessages((all) => ({ ...all, [selected.id]: [...(all[selected.id] || []), { from: 'you', text }] })); setDraft(''); window.setTimeout(() => setMessages((all) => ({ ...all, [selected.id]: [...(all[selected.id] || []), { from: 'agent', text: `Принял задачу. Я разберу её как ${selected.role.toLowerCase()} и вернусь с результатом.` }] })), 450); };
  return <PageNav><section className="teams-page wide"><div className="teams-heading"><div><p className="eyebrow">Рабочее пространство · Команды</p><h1>Твои агенты<br /><em>в одном месте.</em></h1><p>Выбери специалиста, посмотри его инструменты и общайся отдельно — без смешивания контекста.</p></div><div className="teams-summary"><span><Bot size={18} /> {teamAgents.length} агента</span><span><Circle size={10} fill="currentColor" /> Все готовы к работе</span></div></div><div className="teams-shell"><aside className="agents-list"><div className="panel-label"><span>Команда</span><button type="button" aria-label="Добавить агента"><Plus size={16} /></button></div>{teamAgents.map((agent) => <button className={`agent-row ${agent.id === selected.id ? 'selected' : ''}`} key={agent.id} type="button" onClick={() => setSelectedId(agent.id)}><span className="agent-avatar" style={{ background: agent.color }}>{agent.initials}</span><span className="agent-row-copy"><strong>{agent.name}</strong><small>{agent.role}</small></span><span className="online-dot" /></button>)}<div className="agents-hint"><MessageSquare size={15} /><span>У каждого агента<br />свой отдельный диалог</span></div></aside><section className="agent-profile"><div className="profile-top"><span className="profile-avatar" style={{ background: selected.color }}>{selected.initials}</span><div><p className="eyebrow">Профиль агента</p><h2>{selected.name}</h2><span className="profile-status"><Circle size={9} fill="currentColor" /> {selected.status}</span></div></div><p className="agent-description">{selected.description}</p><div className="profile-section"><div className="panel-label"><span>Инструменты</span><Wrench size={15} /></div><div className="tool-list">{selected.tools.map((tool) => <span key={tool}>{tool}</span>)}</div></div><div className="profile-section agent-meta"><div><small>Модель</small><strong>{selected.model}</strong></div><div><small>Роль</small><strong>{selected.role}</strong></div></div></section><section className="agent-chat"><div className="chat-head"><div><p className="eyebrow">Личный диалог</p><h3>{selected.name}</h3></div><span className="chat-secure"><Circle size={8} fill="currentColor" /> отдельно</span></div><div className="chat-messages">{messages[selected.id]?.map((message, index) => <div className={`chat-bubble ${message.from === 'you' ? 'from-you' : ''}`} key={`${message.text}-${index}`}>{message.text}</div>)}</div><div className="chat-compose"><textarea value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); sendMessage(); } }} placeholder={`Напиши ${selected.name.toLowerCase()} задачу...`} rows={2} /><button type="button" onClick={sendMessage} aria-label="Отправить сообщение"><Send size={17} /></button></div><small className="chat-note">Enter — отправить · Shift + Enter — новая строка</small></section></div></section></PageNav>;
}
...
function InnerPage({ path }: { path: string }) {
  const data: Record<string, { kicker: string; title: React.ReactNode; copy: string }> = {
    ...
    '/program': { kicker: 'Программа «Умный путь к стройности»', title: <>Три этапа к<br /><em>своей системе.</em></>, copy: 'Не выдаём жёсткое меню на две недели. Учим рассчитывать порции, выбирать продукты и самостоятельно собирать рацион под свой день.' },
  ...
  return <PageNav><section className={`inner-hero wide${path === '/faq' ? ' faq-page-hero' : ''}`}><Reveal><p className="eyebrow">{current.kicker}</p><h1>{current.title}</h1><p>{current.copy}</p></Reveal></section>{path === '/about' && <AboutContent />}{path === '/program' && <ProgramContent />}{path === '/reviews' && <ReviewsContent />}{path === '/faq' && <FaqContent />}{path === '/application' && <section className="inner-content wide"><div className="application-layout"><div className="application-intro"><p className="eyebrow">Программа «Умный путь к стройности»</p><h2>Заявка на<br /><em>участие.</em></h2><p>Стоимость программы — <strong>6 000 ₽</strong>.</p><ul><li>Личная формула рациона и КБЖУ</li><li>Личный наставник каждый день</li><li>Практика замен и сборки тарелки</li></ul></div><ApplicationForm /></div></section>}{path === '/contacts' && <ContactContent application={false} />}</PageNav>;
}
function AboutContent() { return <section className="inner-content wide"><div className="inner-split"><div><p className="eyebrow">Наталья · 48 лет</p><h2>Нутрициолог<br /><em>и наставник.</em></h2></div><div><p>Я — Наталья, дипломированный нутрициолог и создатель программы «Умный путь к стройности».</p><p>Помогаю девушкам разобраться в белках, жирах и углеводах, рассчитать индивидуальную суточную норму КБЖУ и собрать рацион под вкус, цель и образ жизни.</p><div className="fact-row"><span>48<small>лет опыта жизни и наблюдений за привычками</small></span><span>1 : 1<small>личная обратная связь</small></span><span>3<small>этапа программы</small></span></div></div></div><div className="expertise-band"><div><p className="eyebrow">В программе разбираем</p><h3>Почему голод<br /><em>не всегда про еду.</em></h3></div><p>Говорим о сигналах голода и сытости, роли инсулина, лептина и грелина, простых и сложных углеводах, полезных жирах и белках. Учимся выбирать продукты без тревоги и собирать сбалансированную тарелку.</p></div><div className="knowledge-grid"><article><span>01</span><h3>Хлорофилл и зелень</h3><p>Разберём, чем полезны зелёные овощи и зелень, какие микроэлементы они дают и как полноценный рацион поддерживает кроветворение и доставку кислорода к тканям.</p></article><article><span>02</span><h3>Питьевой режим</h3><p>Научимся распределять воду в течение дня и ориентироваться на жажду, активность, погоду и индивидуальные ограничения.</p></article><article><span>03</span><h3>Способ приготовления</h3><p>Поймём, как учитывать масло, соусы, заправки и термическую обработку, не добавляя в рацион незаметные лишние жиры.</p></article></div><img className="inner-image" src="/gallery/inbound/30.jpg" alt="Корзина со свежими овощами и зеленью" loading="lazy" /></section>; }
...
const faqItems = [
  ['Как проходит программа?', 'Сначала рассчитываем ориентир и разбираем текущий рацион. Затем учимся собирать каждый приём пищи, практикуем замены и каждый день получаем обратную связь.'],
  ...
  ['Подойдёт ли программа без опыта?', 'Да. Всё объясняется от базовых принципов, а Наталья помогает применить их к твоим продуктам и обычному распорядку дня.'],
];
...
function App() {
  ...
  return <main><header className="topbar"><a className="mini-logo" href="/">SSW<span>.</span></a><span className="top-note">Программа «Умный путь к стройности» / 2026</span><div className="top-actions"><a className="nav-link" href="/calculator">Рассчитать КБЖУ <ArrowUpRight size={14} /></a><button className="menu-trigger" type="button" onClick={() => setMenuOpen(true)} aria-label="Открыть меню" aria-expanded={menuOpen}><Menu size={23} strokeWidth={1.5} /></button></div></header>
    ...
    <aside className={`menu-drawer ${menuOpen ? 'is-open' : ''}`} aria-label="Навигация по сайту" aria-hidden={!menuOpen}><button className="menu-close" type="button" onClick={closeMenu} aria-label="Закрыть меню"><X size={23} /></button><p className="eyebrow">Умный путь к стройности</p><p className="drawer-title">Выбери свой<br /><em>следующий шаг.</em></p><nav className="drawer-links"><a href="/" onClick={closeMenu}><span>01</span>Главная <ArrowUpRight size={15} /></a><a href="/teams" onClick={closeMenu}><span>02</span>Команды <ArrowUpRight size={15} /></a><a href="/calculator" onClick={closeMenu}><span>03</span>Расчёт КБЖУ <ArrowUpRight size={15} /></a><a href="/about" onClick={closeMenu}><span>04</span>О Наталье <ArrowUpRight size={15} /></a><a href="/program" onClick={closeMenu}><span>05</span>Программа <ArrowUpRight size={15} /></a><a href="/reviews" onClick={closeMenu}><span>06</span>Результаты <ArrowUpRight size={15} /></a><a href="/faq" onClick={closeMenu}><span>07</span>FAQ <ArrowUpRight size={15} /></a><a href="/contacts" onClick={closeMenu}><span>08</span>Контакты <ArrowUpRight size={15} /></a></nav></aside>
    <CinematicHero /><section className="hero page-column editorial-intro"><Reveal delay={.1}><div className="logo">Умный путь<br />к стройности</div></Reveal><Reveal delay={.2}><div className="eyebrow">Практическая программа питания</div></Reveal><Reveal delay={.3}><h2 className="editorial-title">Не меню на<br />две недели.<br /><em>Навык на жизнь.</em></h2></Reveal><Reveal delay={.4} className="intro"><p>Мы не привязываем тебя к таблице, где сегодня обязательно гречка, а завтра картофель. Учим самостоятельно собирать сбалансированный рацион под свою норму, вкус и распорядок дня.</p><p>Ты поймёшь размеры порций и логику замен: сможешь выбрать картофель, крупу или макароны и сохранить баланс тарелки без чувства, что всё испорчено.</p><p className="intro-motivation">Не нужно становиться идеальной. Нужно научиться выбирать то, что поддерживает тебя каждый день.</p></Reveal><Reveal delay={.5} className="hero-actions"><a className="button secondary" href="#program">Посмотреть, как это работает <ArrowUpRight size={16} /></a></Reveal></section>
    ...
    <section id="about" className="quote-section page-column"><Reveal delay={.1}><Quote size={25} strokeWidth={1.5} /></Reveal><Reveal delay={.2}><blockquote>«Ты не ленивая. Ты просто устала от правил, которые невозможно встроить в жизнь».</blockquote></Reveal></section>
