...
function ApplicationForm() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const maxId = String(form.get('maxId') || '').trim().replace(/^@/, '');
    const data = { createdAt: new Date().toISOString(), program: 'Умный путь к стройности', price: '6000', source: 'smartslimway-max', name: form.get('name') || '', phone: form.get('phone') || '', contactMethod, maxId, email: form.get('email') || '', age: form.get('age') || '', weight: form.get('weight') || '', goal: form.get('goal') || '', experience: form.get('experience') || '', chronicConditions: 'Нет', motivation: form.get('motivation') || '', consent: true };
