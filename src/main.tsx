...
function ApplicationForm() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); setSending(true); setStatus('Отправляю анкету...'); const form = new FormData(event.currentTarget); const telegram = String(form.get('telegram') || '').trim().replace(/^@/, ''); const data = { createdAt: new Date().toISOString(), program: 'Сильные девочки', price: '6000', source: 'smartslimway-site', name: form.get('name') || '', phone: form.get('phone') || '', telegram, email: form.get('email') || '', age: form.get('age') || '', weight: form.get('weight') || '', goal: form.get('goal') || '', experience: form.get('experience') || '', motivation: form.get('motivation') || '' }; try { const saved = JSON.parse(localStorage.getItem('smart-path-applications') || '[]'); localStorage.setItem('smart-path-applications', JSON.stringify([...saved, data])); const controller = new AbortController(); const timeout = window.setTimeout(() => controller.abort(), 8000); const response = await fetch(GOOGLE_SCRIPT_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(data), signal: controller.signal }); window.clearTimeout(timeout); if (!response.ok) throw new Error('sheet'); setStatus('Анкета отправлена. Наталья свяжется с тобой лично.'); event.currentTarget.reset(); } catch { setStatus('Анкета сохранена локально. Проверь подключение к таблице и повтори отправку.'); } finally { setSending(false); } };
  return <form className="application-form" onSubmit={submit}><div className="form-grid"><label>Имя *<input name="name" required placeholder="Твоё имя" /></label><label>Телефон *<input name="phone" type="tel" required placeholder="+7 (999) 123-45-67" /></label><label>Telegram<input name="telegram" placeholder="@username" /></label><label>Email *<input name="email" type="email" required placeholder="твой@email.com" /></label><label>Возраст<input name="age" type="number" min="16" max="99" placeholder="25" /></label><label>Текущий вес<input name="weight" placeholder="65 кг" /></label></div><label>Цель *<input name="goal" required placeholder="Например: наладить режим питания" /></label><fieldset><legend>Опыт с диетами</legend><label><input type="radio" name="experience" value="Новичок" /> Новичок</label><label><input type="radio" name="experience" value="Есть опыт" /> Есть опыт</label><label><input type="radio" name="experience" value="Пробовала много" /> Пробовала много</label></fieldset><label>Почему хочешь участвовать?<textarea name="motivation" rows={4} placeholder="Расскажи о своей ситуации..." /></label><label className="consent"><input type="checkbox" required /> Согласна на обработку персональных данных</label><button className="button primary" type="submit" disabled={sending}>{sending ? 'Отправляю...' : 'Отправить анкету'} <ArrowUpRight size={16} /></button><p className="form-status" role="status">{status}</p></form>;
}
...
const foods: Record<string, Food> = {
  egg: { name: 'Яйца', detail: 'без скорлупы', calories: 157, protein: 12.7, fat: 11.5, carbs: .7 },
  oats: { name: 'Овсяные хлопья', detail: 'сухой вес', calories: 366, protein: 12.3, fat: 6.1, carbs: 61.8 },
  berries: { name: 'Ягоды', detail: 'свежие или замороженные', calories: 45, protein: .8, fat: .4, carbs: 9.5 },
  chicken: { name: 'Куриная грудка', detail: 'готовый вес', calories: 165, protein: 31, fat: 3.6, carbs: 0 },
  quinoa: { name: 'Киноа', detail: 'готовый вес', calories: 120, protein: 4.4, fat: 1.9, carbs: 21.3 },
  salad: { name: 'Овощной салат', detail: 'огурец, томат, зелень', calories: 27, protein: 1.2, fat: .3, carbs: 4.8 },
  oliveOil: { name: 'Оливковое масло', detail: 'заправка для салата', calories: 899, protein: 0, fat: 99.9, carbs: 0 },
  fish: { name: 'Белая рыба', detail: 'готовый вес', calories: 120, protein: 24, fat: 2.5, carbs: 0 },
  potato: { name: 'Запечённый картофель', detail: 'готовый вес', calories: 93, protein: 2.5, fat: .1, carbs: 21 },
  vegetables: { name: 'Запечённые овощи', detail: 'кабачок, перец, брокколи', calories: 42, protein: 2, fat: .5, carbs: 7.5 },
  yogurt: { name: 'Греческий йогурт 2%', calories: 73, protein: 10, fat: 2, carbs: 3.6 },
  apple: { name: 'Яблоко', calories: 52, protein: .3, fat: .2, carbs: 13.8 },
  almonds: { name: 'Миндаль', calories: 609, protein: 18.6, fat: 53.7, carbs: 13 },
  cottageCheese: { name: 'Творог 5%', calories: 145, protein: 21, fat: 5, carbs: 3 },
  banana: { name: 'Банан', calories: 96, protein: 1.5, fat: .5, carbs: 21 },
};
...
function buildMealPlan(target: Nutrients, mealCount: number): { meals: PlannedMeal[]; nutrients: Nutrients } {
  ...
  const templates: Record<string, { title: string; portions: [Food, number, number?][] }> = {
    ...
    lunch: { title: 'Обед', portions: [[foods.chicken, 150], [foods.quinoa, 160], [foods.salad, 250], [foods.oliveOil, 10, 1]] },
    ...
    dinner: { title: 'Ужин', portions: [[foods.fish, 170], [foods.potato, 220], [foods.vegetables, 250], [foods.oliveOil, 8, 1]] },
  };
...
function InnerPage({ path }: { path: string }) {
  const data: Record<string, { kicker: string; title: React.ReactNode; copy: string }> = {
    ...
    '/program': { kicker: 'Марафон «Сильные девочки»', title: <>Три этапа к<br /><em>своей системе.</em></>, copy: 'Не выдаём жёсткое меню на две недели. Учим рассчитывать порции, выбирать продукты и самостоятельно собирать рацион под свой день.' },
  ...
  return <PageNav><section className="inner-hero wide"><Reveal><p className="eyebrow">{current.kicker}</p><h1>{current.title}</h1><p>{current.copy}</p></Reveal></section>{path === '/about' && <AboutContent />}{path === '/program' && <ProgramContent />}{path === '/reviews' && <ReviewsContent />}{path === '/faq' && <FaqContent />}{path === '/application' && <section className="inner-content wide"><div className="application-layout"><div className="application-intro"><p className="eyebrow">Марафон «Сильные девочки»</p><h2>Заявка на<br /><em>участие.</em></h2><p>Стоимость программы — <strong>6 000 ₽</strong>.</p><ul><li>Личная формула рациона и КБЖУ</li><li>Личный ментор каждый день</li><li>Практика замен и сборки тарелки</li></ul></div><ApplicationForm /></div></section>}{path === '/contacts' && <ContactContent application={false} />}</PageNav>;
}
function AboutContent() { return <section className="inner-content wide"><div className="inner-split"><div><p className="eyebrow">Наталья · 48 лет</p><h2>Нутрициолог<br /><em>и ментор.</em></h2></div><div><p>Я — Наталья, дипломированный нутрициолог и создатель марафона «Сильные девочки».</p><p>Помогаю девушкам разобраться в белках, жирах и углеводах, рассчитать индивидуальную суточную норму КБЖУ и собрать рацион под вкус, цель и образ жизни.</p><div className="fact-row"><span>48<small>лет опыта жизни и наблюдений за привычками</small></span><span>1 : 1<small>личная обратная связь</small></span><span>3<small>этапа программы</small></span></div></div></div><div className="expertise-band"><div><p className="eyebrow">На марафоне разбираем</p><h3>Почему голод<br /><em>не всегда про еду.</em></h3></div><p>Говорим о сигналах голода и сытости, роли инсулина, лептина и грелина, простых и сложных углеводах, полезных жирах и белках. Учимся выбирать продукты без тревоги и собирать сбалансированную тарелку.</p></div><img className="inner-image" src={gallery[1]} alt="Полезное блюдо на кухне" /></section>; }
function ProgramContent() { return <section className="inner-content wide"><div className="inner-section-title"><p className="eyebrow">Что входит</p><h2>Понятный маршрут<br /><em>без крайностей.</em></h2></div><div className="inner-stage-grid">{stages.map((stage, i) => <article className={i === 1 ? 'dark-plan' : ''} key={stage[0]}><span>Этап 0{i + 1}</span><h3>{stage[0]}</h3><p>{stage[1]}</p><strong>{stage[2]}</strong></article>)}</div><div className="program-details"><p className="eyebrow">Внутри программы</p><p>На примерах разбираем состав продуктов, размеры порций и заправки. Учимся заменять продукты внутри своей нормы, а не следовать списку «в понедельник грудка и гречка».</p><p>Главный результат — ты можешь открыть холодильник и собрать сбалансированный приём пищи из того, что хочется сегодня. Поддержка помогает закрепить этот навык в обычной жизни.</p></div><blockquote className="program-quote">«Свобода в питании начинается не с идеального меню, а с понимания, как выбрать то, что подходит тебе сегодня».</blockquote></section>; }
...
function FaqContent() { return <section className="inner-content wide faq-experience"><div className="faq-intro"><p className="eyebrow">Без неудобных вопросов не бывает хорошего решения</p><blockquote>«Тебе не нужно заранее всё знать. Достаточно быть готовой разобраться в себе без осуждения».</blockquote></div><div className="faq-grid full-faq">{faqItems.map(([question, answer], index) => <details open={index === 0} key={question}><summary><span>0{index + 1}</span>{question}<ArrowUpRight size={18} /></summary><p>{answer}</p></details>)}</div></section>; }
function ContactContent({ application }: { application: boolean }) { return <section className="inner-content wide"><div className="contact-card contact-card-rich"><div><p className="eyebrow">{application ? 'Марафон «Сильные девочки»' : 'Личный контакт'}</p><h2>{application ? <>Оставь заявку<br /><em>на участие.</em></> : <>Начни не с диеты.<br /><em>Начни с разговора.</em></>}</h2></div><div className="contact-card-copy"><p>Анкета — это не обязательство купить курс. Наталья прочитает ответы лично, уточнит твою цель и честно скажет, подходит ли такой формат.</p><blockquote>«Хороший план начинается не со списка продуктов, а с понимания человека».</blockquote><a className="text-link" href="/application">Рассказать о своей ситуации <ArrowUpRight size={16} /></a></div></div></section>; }
function App() {
  ...
  return <main><header className="topbar"><a className="mini-logo" href="/">SSW<span>.</span></a><span className="top-note">Марафон «Сильные девочки» / 2026</span><div className="top-actions"><a className="nav-link" href="/calculator">Рассчитать КБЖУ <ArrowUpRight size={14} /></a><button className="menu-trigger" type="button" onClick={() => setMenuOpen(true)} aria-label="Открыть меню" aria-expanded={menuOpen}><Menu size={23} strokeWidth={1.5} /></button></div></header>
    ...
    <CinematicHero /><section className="hero page-column editorial-intro"><Reveal delay={.1}><div className="logo">Умный путь<br />к стройности</div></Reveal><Reveal delay={.2}><div className="eyebrow">Практический марафон питания · Наталья, 48 лет</div></Reveal><Reveal delay={.3}><h2 className="editorial-title">Не меню на<br />две недели.<br /><em>Навык на жизнь.</em></h2></Reveal><Reveal delay={.4} className="intro"><p>Мы не привязываем тебя к таблице, где сегодня обязательно гречка, а завтра картофель. Учим самостоятельно собирать сбалансированный рацион под свою норму, вкус и распорядок дня.</p><p>Ты поймёшь размеры порций и логику замен: сможешь выбрать картофель, крупу или макароны и сохранить баланс тарелки без чувства, что всё испорчено.</p><p className="intro-motivation">Не нужно становиться идеальной. Нужно научиться выбирать то, что поддерживает тебя каждый день.</p></Reveal><Reveal delay={.5} className="hero-actions"><a className="button secondary" href="#program">Посмотреть, как это работает <ArrowUpRight size={16} /></a></Reveal></section>
    ...
    <section id="faq" className="faq wide"><Reveal><p className="eyebrow">04 / вопросы</p><h2>Сомнения —<br /><em>это нормально.</em></h2><p className="faq-lead">Марафон не требует идеальной дисциплины. Он нужен, чтобы научиться принимать понятные решения даже в неидеальный день.</p></Reveal><div className="faq-grid">{faqItems.slice(0, 4).map(([question, answer], index) => <details key={question}><summary><span>0{index + 1}</span>{question}<ArrowUpRight size={18} /></summary><p>{answer}</p></details>)}</div></section>
...
createRoot(document.getElementById('root')!).render(<StrictMode>{path === '/' ? <App /> : <InnerPage path={path} />}</StrictMode>);
