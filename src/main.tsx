...
// SmartSlimWay original photos (PRIORITY 1–3 from Designer manifest 2026-08-06) mixed in.
const gallery = [
  ...([5, 6, 7, 9, 10, 23, 26, 27, 29, 34, 35]
    .map((index) => `/gallery/inbound/${String(index).padStart(2, '0')}.jpg`)),
  '/gallery/smartslimway/gallery-1086.jpg',
  '/gallery/smartslimway/gallery-1111.jpg',
  '/gallery/smartslimway/gallery-1440.jpg',
  '/gallery/smartslimway/gallery-1445.jpg',
  '/gallery/smartslimway/gallery-1652.jpg',
];
const testimonials = [
...
function ParallaxImage() {
  ...
  return <div ref={ref} className="parallax-frame"><img src="/gallery/smartslimway/about-1593.jpg" alt="Наталья — нутрициолог и наставник программы SmartSlimWay" style={{ transform: `translateY(${offset}px) scale(1.08)` }} /></div>;
}
...
const goalCopy: Record<NutritionGoal, { title: string; adjustment: number; protein: number; image: string; note: string }> = {
  loss: { title: 'Снижение веса', adjustment: -0.17, protein: 1.8, image: '/gallery/smartslimway/program-1550.jpg', note: 'Умеренный дефицит около 17% от поддержки — без экстремальных ограничений.' },
  maintenance: { title: 'Поддержание', adjustment: 0, protein: 1.8, image: '/gallery/smartslimway/program-1550.jpg', note: 'Ориентир для сохранения текущего веса и стабильной энергии.' },
  gain: { title: 'Набор массы', adjustment: 0.1, protein: 1.8, image: '/gallery/smartslimway/program-1550.jpg', note: 'Небольшой профицит около 10% от поддержки с акцентом на белок.' },
};
...
function App() {
  ...
  return <main><header className="topbar"><a className="mini-logo" href="/">SSW<span>.</span></a><span className="top-note">Программа «Умный путь к стройности» / 2026</span><div className="top-actions"><a className="nav-link" href="/calculator">Рассчитать КБЖУ <ArrowUpRight size={14} /></a><button className="menu-trigger" type="button" onClick={() => setMenuOpen(true)} aria-label="Открыть меню" aria-expanded={menuOpen}><Menu size={23} strokeWidth={1.5} /></button></div></header>
    ...
    <section className="case-studies wide"><Reveal><p className="eyebrow">Что входит в программу</p><h2>Система, которая<br /><em>остаётся с тобой.</em></h2></Reveal><div className="case-list">{[['Твоя формула тарелки', 'Понятные порции белка, гарнира, овощей и жиров под твою цель', '/gallery/smartslimway/program-1550.jpg'], ['Практика замен', 'Картофель, гречка или макароны — учимся выбирать без страха и запретов', '/gallery/smartslimway/program-1623.jpg'], ['Ежедневная связь', 'Личный наставник, разбор тарелок, обратная связь и групповой чат', '/gallery/smartslimway/gallery-1652.jpg']].map((item, i) => <Reveal key={item[0]} delay={i * .1}><article className="case-item"><div className="case-label"><h3>{item[0]}</h3><p>{item[1]}</p></div><img src={item[2]} alt={item[0]} loading="lazy" /></article></Reveal>)}</div></section>
