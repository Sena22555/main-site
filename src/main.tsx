...
const gallery = [
  ...[1, 2, 4, 5, 6, 7, 8, 9, 10, 12, 15, 16, 17, 18, 19, 20, 21, 23, 25, 26, 27, 29, 30, 31, 32, 33, 34, 35, 36].map((index) => `/gallery/inbound/${String(index).padStart(2, '0')}.jpg`),
];
const testimonials = [
...
function App() {
  ...
  return <main><header className="topbar"><a className="mini-logo" href="/">SSW<span>.</span></a><span className="top-note">Программа «Умный путь к стройности» / 2026</span><div className="top-actions"><a className="nav-link" href="/calculator">Рассчитать КБЖУ <ArrowUpRight size={14} /></a><button className="menu-trigger" type="button" onClick={() => setMenuOpen(true)} aria-label="Открыть меню" aria-expanded={menuOpen}><Menu size={23} strokeWidth={1.5} /></button></div></header>
    ...
    <section className="case-studies wide"><Reveal><p className="eyebrow">Что входит в программу</p><h2>Система, которая<br /><em>остаётся с тобой.</em></h2></Reveal><div className="case-list">{[['Твоя формула тарелки', 'Понятные порции белка, гарнира, овощей и жиров под твою цель', '/gallery/inbound/01.jpg'], ['Практика замен', 'Картофель, гречка или макароны — учимся выбирать без страха и запретов', '/gallery/inbound/04.jpg'], ['Ежедневная связь', 'Личный наставник, разбор тарелок, обратная связь и групповой чат', '/gallery/inbound/07.jpg']].map((item, i) => <Reveal key={item[0]} delay={i * .1}><article className="case-item"><div className="case-label"><h3>{item[0]}</h3><p>{item[1]}</p></div><img src={item[2]} alt={item[0]} loading="lazy" /></article></Reveal>)}</div></section>
