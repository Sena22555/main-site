...
function CinematicHero() {
  ...
  return <section id="top" className="cinematic-hero" onPointerDown={startOnTouch} onClick={startOnTouch}><video ref={videoRef} className="cinematic-video" autoPlay muted loop playsInline disablePictureInPicture preload="auto" poster="/hero-first-frame.jpg" onPlaying={() => setVideoReady(true)} onLoadedMetadata={(event) => { event.currentTarget.muted = true; event.currentTarget.defaultMuted = true; event.currentTarget.playbackRate = 1.25; }}><source src="/hero-video.mp4" type="video/mp4" /></video><img className="cinematic-motion" src="/hero-motion.webp" alt="" aria-hidden="true" /><img className={`cinematic-poster ${videoReady ? 'is-hidden' : ''}`} src="/hero-first-frame.jpg" alt="" aria-hidden="true" /><div className="cinematic-shade" /><div className="cinematic-copy"><Reveal delay={.1}><p className="cinematic-kicker">Марафон «Сильные девочки» · SmartSlimWay</p></Reveal><Reveal delay={.2}><h1>Питание<br /><em>без крайностей.</em></h1></Reveal><Reveal delay={.3}><p className="cinematic-description">Собери свою систему питания — в своём ритме, с поддержкой и без вечной войны с собой.</p></Reveal><Reveal delay={.4}><a className="cinematic-cta" href="/application">Подать заявку <ArrowUpRight size={16} /></a></Reveal></div><div className="cinematic-bottom"><span>Три этапа · Личный наставник · Онлайн-анкета</span><span className="cinematic-scroll">Scroll to explore ↓</span></div></section>;
}
function Reviews() {
